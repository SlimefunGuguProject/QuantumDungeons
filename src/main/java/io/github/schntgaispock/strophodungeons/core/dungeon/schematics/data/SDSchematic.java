...
@ToString
@Getter
public class SDSchematic {

    private final int[][][] blocks;
    private final List<SDSBlockState> palette;
    private final Biome biome;
    private final List<SDSFace> faces;
    private final int version;

    @JsonCreator(mode = Mode.PROPERTIES)
    public SDSchematic(
            @JsonProperty("blocks") int[][][] blocks,
            @JsonProperty("palette") List<SDSBlockState> palette,
            @JsonProperty("biome") Biome biome,
            @JsonProperty("faces") List<SDSFace> faces,
        ...
        for (int x = 0; x < xLength; x++) {
            ...
            for (int y = 0; y < yLength; y++) {
                ...
                for (int z = 0; z < zLength; z++) {
                    Location l = new Location(world, cornerX + x, cornerY + y, cornerZ + z);
                    ...
                    SDSBlockState data = new SDSBlockState(b.getBlockData(), l);
        ...
        for (int x = x0; px.test(x); x += dx) {
            ...
            for (int y = 0; y < yLen; y++) {
                ...
                for (int z = z0; pz.test(z); z += dz) {
                    ...
                    SDSBlockState state = palette.get(stateId - 1);
                    ...
                    if (slimefunId != null) {
                        SDBlockStorage.setSlimefunBlock(b.getLocation(), slimefunId);
                    }
